...
def createUtf8TestSequences():
   ...
   for z1 in [b'\xf0', b'\xf1', b'\xf2', b'\xf3', b'\xf4']:
      for z2 in [b'\x8f', b'\x9f', b'\xaf', b'\xbf']:
         if not (z1 == b'\xf4' and z2 != b'\x8f'): # those encode codepoints >U+10FFFF
            for z3 in [b'\xbe', b'\xbf']:
               zz = z1 + z2 + b'\xbf' + z3
               if zz not in [b'\xf0\x8f\xbf\xbe', b'\xf0\x8f\xbf\xbf']: # filter overlong sequences
                  vs[1].append((True, zz))
   UTF8_TEST_SEQUENCES.append(vs)
...
def test_utf8_incremental(validator, withPositions = True):
   ...
   validator.reset()
   assert (True, True, 6, 6)[:k] == validator.validate(b"\x65\x64\x69\x74\x65\x64")[:k]
   assert (False, False, 1, 7)[:k] == validator.validate(b"\xED\xA0\x80")[:k]

   validator.reset()
   assert (True, True, 4, 4)[:k] == validator.validate(b"\x65\x64\x69\x74")[:k]
   assert (False, False, 3, 7)[:k] == validator.validate(b"\x65\x64\xED\xA0\x80")[:k]

   validator.reset()
   assert (True, False, 7, 7)[:k] == validator.validate(b"\x65\x64\x69\x74\x65\x64\xED")[:k]
...
for t in createUtf8TestSequences():
   ...
   for p in t[1]:
      ...
      C = type("Case6_%d_%d" % (i, j),
                ...
                {"PAYLOAD": p[1],
                 "isValid": p[0],
                 "DESCRIPTION": """%s<br><br>Payload: 0x%s""" % (desc, binascii.b2a_hex(p[1]).decode('ascii')),
                 "EXPECTATION": """%s""" % exp,
                 "__init__": __init__,
                 "onOpen": onOpen})
      Case6_X_X.append(C)
...
def encode(c):
   ...
   if c <= 0x7F:
      b1 = c>>0  & 0x7F | 0x00
      return array.array('B', [b1]).tobytes()
   elif c <= 0x07FF:
      b1 = c>>6  & 0x1F | 0xC0
      b2 = c>>0  & 0x3F | 0x80
      return array.array('B', [b1, b2]).tobytes()
   elif c <= 0xFFFF:
      b1 = c>>12 & 0x0F | 0xE0
      b2 = c>>6  & 0x3F | 0x80
      b3 = c>>0  & 0x3F | 0x80
      return array.array('B', [b1, b2, b3]).tobytes()
   elif c <= 0x1FFFFF:
      b1 = c>>18 & 0x07 | 0xF0
      b2 = c>>12 & 0x3F | 0x80
      b3 = c>>6  & 0x3F | 0x80
      b4 = c>>0  & 0x3F | 0x80
      return array.array('B', [b1, b2, b3, b4]).tobytes()
   elif c <= 0x3FFFFFF:
      b1 = c>>24 & 0x03 | 0xF8
      b2 = c>>18 & 0x3F | 0x80
      b3 = c>>12 & 0x3F | 0x80
      b4 = c>>6  & 0x3F | 0x80
      b5 = c>>0  & 0x3F | 0x80
      return array.array('B', [b1, b2, b3, b4, b5]).tobytes()
   elif c <= 0x7FFFFFFF:
      b1 = c>>30 & 0x01 | 0xFC
      b2 = c>>24 & 0x3F | 0x80
      b3 = c>>18 & 0x3F | 0x80
      ...
      b5 = c>>6  & 0x3F | 0x80
      b6 = c>>0  & 0x3F | 0x80
      return array.array('B', [b1, b2, b3, b4, b5, b6]).tobytes()
   else:
